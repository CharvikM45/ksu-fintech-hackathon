...
def seed_demo_data():
    ...
    for user in demo_users:
        ...
        cursor.execute(
            ...
            (user['id'], user['name'], user['phone'], pin_hash, user['balance'], user['is_vendor'], qr_data)
        )
    ...
    for txn in demo_txns:
        cursor.execute(
            "INSERT INTO transactions (id, sender_id, receiver_id, amount, type, note, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)",
            (txn['id'], txn['sender_id'], txn['receiver_id'], txn['amount'], txn['type'], txn['note'], txn['created_at'])
        )
        # Create receipt
        sender = next(u for u in demo_users if u['id'] == txn['sender_id'])
        receiver = next(u for u in demo_users if u['id'] == txn['receiver_id'])
        cursor.execute(
            "INSERT INTO receipts (id, transaction_id, sender_name, receiver_name, amount, type, status, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
            (f'RCP-{uuid.uuid4().hex[:8].upper()}', txn['id'], sender['name'], receiver['name'], txn['amount'], txn['type'], 'completed', txn['created_at'])
        )
